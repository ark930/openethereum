...
use {trie_vm_factories, journaldb, kvdb_memorydb};
use kvdb::{self, KeyValueDB};
use {state_db, trace, spec};
use pod::PodState;
use common_types::{
	errors::EthcoreError,
	log_entry,
...
impl<'a> EvmTestClient<'a> {
	...
	fn state_from_pod(spec: &'a spec::Spec, factories: &Factories, pod_state: PodState) -> Result<State<state_db::StateDB>, EvmTestError> {
		...
		let client_path = Path::new("./data");
		let mut state = super::get_temp_state_rockdb(client_path);
		state.populate_from(pod_state);
		state.commit()?;
		// state.drop();
		Ok(state)
